...
class Beaneater
  ...
  class Connection
    ...
    # Returns the new communication socket
    #
    # If the connection is already setup returns the current socket otherwise
    # tries to get a new one.
    # @return [Net::TCPSocket] the connection socket object
    #
    def connection
    ...
    def _with_retry(retry_interval=1, &block)
      yield
    rescue DrainingError, EOFError, Errno::ECONNRESET, Errno::EPIPE, Errno::ECONNREFUSED => ex
    ...
    def _reconnect(original_exception, retry_interval=1, tries=MAX_RETRIES)
      ...
      tries -= 1
      raise original_exception if tries.zero?
